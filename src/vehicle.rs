...
use crate::{
    util::{self, combine_vec_u128, g2_to_vec_u128, gen_random_fr, mul_g1_fr, CertV, IAPublicKey},
    BLS, DAE, DGSA, PKE, SE,
};
use aes_gcm_siv::{
...
impl Veh {
    ...
    pub fn new() -> Self {
        Veh {
            v_id: 0,
            v_sk: Fr::zero(), // Initialize sk to zero (or any default value)
            v_pk: G2::zero(), // Initialize pk to zero (or any default value)
            sig_e: G1::zero(),
    ...
    pub fn get_cred(
        ...
        let result = DGSA::issue_v(&sigma, &self.v_id, &epoch, self.v_pk, IAPublicKey);
