...
use crate::util::{
    self, add_g1_g1, add_g2_g2, do_pairing, gen_random_fr, minus_fr_fr, minus_g1_g1, mul_fr_fr,
    mul_g1_fr, mul_g2_fr, Hash_into_Fr, PS_pk, PS_sk,
};

pub fn keygen(rng: &mut XorShiftRng, k: u64) -> (PS_sk, PS_pk) {
    let g = G1::one();
    let g_dash = G2::one();
...
pub fn GenCommitment(
    ...
    let pie_r = gen_random_fr(rng);

    for i in 0..k {
        let rt_i = gen_random_fr(rng);
        pie_rt.push(rt_i);
    }
